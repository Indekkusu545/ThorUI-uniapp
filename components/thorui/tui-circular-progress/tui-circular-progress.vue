<template>
  ...
  >
    <!-- #ifndef MP-ALIPAY || MP-WEIXIN || MP-QQ -->
    <canvas
...
export default {
  ...
  methods: {
    ...
    //默认(背景)圆环
    async drawDefaultCircular() {
      const ctx = await this.getContext(this.defaultCanvasId);
    ...
    //进度圆环
    async drawProgressCircular(startPercentage) {
      let ctx = this.progressContext;
      let gradient = this.linearGradient;
      if (!ctx) {
        ctx = await this.getContext(this.progressCanvasId);
        ...
        if (
          !this.gradualColor &&
          res.platform.toLocaleLowerCase() === "android"
        ) {
      ...
      // #endif
      // #ifdef MP-WEIXIN || MP-QQ
      ctx.clearRect(0, 0, this.diam, this.height || this.diam);
      ctx.lineWidth = lineWidth;
      ctx.strokeStyle = gradient;
      // #endif
      let time =
        this.percentage === 0 || this.duration < 50
          ? 0
          : this.duration / this.percentage;
      if (this.percentage > 0) {
        startPercentage =
          this.duration < 50 ? this.percentage - 1 : startPercentage;
        startPercentage++;
      }
      if (this.fontShow) {
      ...
      if (
        this.percentage === 0 ||
        (this.counterclockwise && startPercentage === 100)
      ) {
        // #ifndef MP-WEIXIN || MP-QQ
        ctx.draw();
        // #endif
      ...
      setTimeout(async () => {
        ...
        if (startPercentage === this.percentage) {
          ...
          await this.drawProgressCircular(startPercentage);
        }
        this.$emit("change", {
          percentage: startPercentage,
        });
